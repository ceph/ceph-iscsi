...
class RBDDev(object):
    ...
    def rbd_size(self):
        ...
        passed in the ansible config file - if the required size is > than
    ...
    def unmap_rbd(self):
        ...
        resp = shellcommand("rbd -c {} unmap {}".format(settings.config.cephconf,
                                                        rbd_path))
        if resp != '':
            self.error = True
            return

        # unmap'd from runtime, now remove from the rbdmap file referenced
        # at boot
        for rbdmap_entry in fileinput.input(settings.config.rbd_map_file,
                                            inplace=True):
    ...
    def get_rbd_map(self):
        ...
        # Now look at mapping of the device - which would execute on all
        # target hosts
        ...
        # Check the showmapped output for this rbd image, and if so set the
        # mapped device name
        ...
        map_cmd = ('rbd -c {} map -o noshare,lock_on_read '
                   '{}/{} 2> /dev/null'.format(settings.config.cephconf,
                                               self.pool,
                                               self.image))
        response = shellcommand(map_cmd)
        if response is None:
            map_cmd = ('rbd -c {} map -o noshare '
                       '{}/{} 2> /dev/null'.format(settings.config.cephconf,
                                                   self.pool,
                                                   self.image))
            response = shellcommand(map_cmd)
...
class LUN(object):
    ...
    def remove_lun(self):
        ...
        if not rbd_image.rbd_map:
            self.error = True
            self.error_msg = ("Unable to get the map device for "
                              "{}/{}".format(rbd_image.pool, rbd_image.image))
        ...
        if LUN.remove_dm_device(dm_path):
            ...
            if rbd_image.error:
                self.error = True
                self.error_msg = ("Unable to unmap {} from "
                                  "host".format(self.config_key))
                self.logger.error(self.error_msg)
                return
            ...
            self.error = True
            self.error_msg = ("Unable to remove dm device for "
                              "{}".format(self.config_key))
            self.logger.error(self.error_msg)
            return
    ...
    def allocate(self):
        self.logger.debug("LUN.allocate starting, listing of rbd devices")
        ...
        if self.num_changes == 0 and this_host == self.allocating_host:
            ...
            if rbd_image.changed:
                self.logger.info("rbd image {} resized to {}".format(self.image,
                                                                     self.size))
                self.num_changes += 1
            else:
                self.logger.debug("rbd image {} size matches the configuration"
                                  " file request".format(self.image))

        # for LIO mapping purposes, we use the device mapper device not the
        # raw /dev/rbdX device. Using the dm device ensures that any
        # connectivity issue doesn't result in stale device structures in the
        # kernel, since device-mapper will tidy those up
        self.dm_get_device(rbd_image.rbd_map)
        if self.dm_device is None:
            self.logger.critical("Could not find dm multipath device for {}. "
                                 "Make sure the multipathd service is enabled,"
                                 " and confirm entry is in "
                                 "/dev/mapper/".format(self.image))
            self.error = True
            self.error_msg = ("Could not find dm multipath device for "
                              "{}".format(self.image))
            return
        ...
        if not self.dm_size_ok(rbd_image):
            self.error = True
            self.error_msg = ("Unable to sync the dm device to the parent "
                              "rbd size - {}".format(self.image))
            self.logger.critical(self.error_msg)
            return

        self.logger.debug("Begin processing LIO mapping requirement")

        self.logger.debug("(LUN.allocate) {} is mapped to "
                          "{}.".format(self.image,
                                       self.dm_device))

        # check this rbd image is in the /etc/ceph/rbdmap file
        if rbd_image.rbdmap_entry():
            self.logger.debug('(LUN.allocate) Entry added to '
                              '/etc/ceph/rbdmap for {}/{}'.format(self.pool,
                                                                  self.image))
            self.num_changes += 1
    ...
    def dm_size_ok(self, rbd_object):
        """
        Check that the dm device matches the request. if the size request is
        lower than current size, just return since resizing down is not support
        and problematic for client filesystems anyway
        ...
        # been resized on another gateway host when this is called from Ansible
        ...
        # since the size matches underneath device mapper, now we ensure the
        # size matches with device mapper - if not issue a resize map request
        if rbd_size_ok:

            # find the dm-X device
            dm_devices = glob.glob('/sys/class/block/dm-*/')
            # convert the full dm_device path to just the name
            ...
            if dm_path_found:
                ...
                if dm_size_bytes != target_bytes:
                    ...
                    self.logger.info("Issuing a resize map for "
                                     "{}".format(dm_name))

                    response = shellcommand('multipathd resize map '
                                            '{}'.format(dm_name))
                    ...
                    if response.lower().startswith('ok') and dm_size_bytes == target_bytes:
                        ...
                        self.logger.critical("multipathd resize map for "
                                             "{} failed".format(dm_name))
                else:
                    # size matches
                    size_ok = True
            else:
                self.logger.critical("Unable to locate a dm-X device for this "
                                     "rbd image - {}".format(self.image))
    ...
    def add_dev_to_lio(self, in_wwn=None):
        ...
        self.logger.info("(LUN.add_dev_to_lio) Adding image '{}' with path "
